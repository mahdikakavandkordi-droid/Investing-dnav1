...
// Intentionally stale/contradictory row: the run says context_required while
// the row still carries a numeric score and eligible/top_match labels. The UI
// must treat the run status as authoritative and never expose 82/100 here.
const dnaOnlyMatch={
...
(async()=>{
 ...
 await ctx.route('https://dna-test.supabase.co/**',async route=>{
  ...
  if(url.includes('/functions/v1/investor-dna-report')){
   ...
   if(body.action==='pdf'){pdfRequests++;assert.equal(body.assessment_id,'assessment-1');assert.equal(body.session_token,'guest-capability');return send({filename:'investor-dna-report-test.pdf',mime_type:'application/pdf',base64:Buffer.from('%PDF-1.4 test').toString('base64')});}
 ...
 let resultText=await page.locator('body').innerText();assert.match(resultText,/Mahdi/);assert.match(resultText,/age/i);assert.match(resultText,/risk tolerance/i);assert.match(resultText,/financial capacity/i);assert.match(resultText,/Your decision fingerprint/i);assert.match(resultText,/cognitive & emotional bias signals/i);assert.match(resultText,/Worth watching/i);assert.match(resultText,/Educational self-assessment only/i);assert.match(resultText,/DNA-only ETF comparisons/i);assert.doesNotMatch(resultText,/82\/100/);
 assert.equal(await page.evaluate(key=>localStorage.getItem(key),DRAFT_KEY),null);assert.ok(await page.evaluate(key=>localStorage.getItem(key),CLAIM_KEY));assert.ok(await page.evaluate(key=>sessionStorage.getItem(key),EPHEMERAL_RESULT_KEY));
 ...
 assert.ok(await page.getByRole('button',{name:'Download PDF'}).count());assert.ok(await page.getByRole('button',{name:'Print'}).count());assert.ok(await page.getByRole('button',{name:'Email PDF'}).count());
 await page.getByRole('button',{name:'Download PDF'}).click();await page.getByRole('button',{name:'Download PDF'}).waitFor();assert.equal(pdfRequests,1);
 check('submission retry reaches personalized ephemeral report, exposes guest-safe report export and redacts stale numeric Match data before context');

 await page.reload();await page.getByRole('heading',{name:'MAVERICK'}).waitFor();resultText=await page.locator('body').innerText();assert.match(resultText,/Mahdi/);assert.match(resultText,/DNA-only ETF comparisons/i);assert.doesNotMatch(resultText,/82\/100/);check('guest report survives refresh without exposing a stale pre-context score');

 await page.getByRole('link',{name:'See all DNA matches'}).click();await page.waitForURL(url=>url.pathname==='/match');const matchHeading=page.locator('.match-dna-hero h1');await matchHeading.waitFor();assert.match(await matchHeading.innerText(),/See how your DNA lines up with ETFs|Your ETF matches/);await page.getByText('VFV',{exact:true}).first().waitFor();const dnaOnlyText=await page.locator('body').innerText();assert.doesNotMatch(dnaOnlyText,/Sign in to see compatibility/);assert.match(dnaOnlyText,/DNA-only/i);assert.doesNotMatch(dnaOnlyText,/82\/100/);check('same-session guest sees DNA-only Match with global status overriding stale numeric row data');
 await page.getByRole('link',{name:'Add investment context'}).first().click();await page.waitForURL(url=>url.pathname==='/dna/context'&&url.searchParams.get('returnTo')==='/match');
 ...
 await page.waitForURL(url=>url.pathname==='/match');await page.getByRole('heading',{name:'Review this money before ranking ETFs'}).waitFor();assert.equal(contextSaves,1);assert.match(await page.locator('body').innerText(),/Matching paused/i);assert.doesNotMatch(await page.locator('body').innerText(),/82\/100/);assert.equal(await page.locator('.match-dna-card').count(),0);check('goal context preserves identity, saves all core constraints, and fully pauses stale ranking cards under review');

 await page.goto(ORIGIN+'/dna');await page.waitForURL(url=>url.pathname==='/dna');await page.getByRole('link',{name:'View my current DNA',exact:true}).click();await page.waitForURL(url=>url.pathname==='/dna/result');await page.getByRole('heading',{name:/Mahdi, a clearer path for this goal\./}).waitFor();const appliedText=await page.locator('body').innerText();assert.match(appliedText,/MAVERICK/);assert.match(appliedText,/same DNA/i);assert.match(appliedText,/Principal protection/i);assert.match(appliedText,/Part 3 · Portfolio Blueprint/i);assert.match(appliedText,/Your Core Blueprint/i);assert.match(appliedText,/0%\s*Equity/i);assert.match(appliedText,/Constraint capped/i);assert.match(appliedText,/DNA Match paused/i);check('applied report keeps identity and archetype, adds a protection-constrained blueprint, and preserves the Match stop');

 const feedback=await ctx.newPage();await feedback.goto(ORIGIN+'/feedback');const groups=feedback.locator('fieldset');await groups.nth(0).getByRole('button',{name:'4',exact:true}).click();await groups.nth(1).getByRole('button',{name:'4',exact:true}).click();await groups.nth(2).getByRole('button',{name:'5',exact:true}).click();await groups.nth(3).getByRole('button',{name:'Yes',exact:true}).click();await groups.nth(4).getByRole('button',{name:'No',exact:true}).click();await groups.nth(5).getByRole('button',{name:'No',exact:true}).click();await groups.nth(6).getByRole('button',{name:'Yes',exact:true}).click();await feedback.getByLabel(/What was confusing or missing/).fill('Clear enough for a pilot test.');await feedback.getByRole('button',{name:'Send pilot feedback'}).click();await feedback.getByRole('heading',{name:/Thank you/}).waitFor();assert.equal(feedbacks,1);await feedback.close();check('structured pilot feedback submits without changing the DNA result');

 await page.getByRole('button',{name:'Save my report'}).click();await page.getByRole('dialog',{name:'Save your Investor DNA'}).waitFor();assert.equal(await page.getByLabel('First name *').inputValue(),'Mahdi');const saveModalText=await page.locator('.save-modal').innerText();assert.match(saveModalText,/Email and first name are enough to create the account/);assert.doesNotMatch(saveModalText,/Last name \(optional\)|Phone \(optional\)/);
 await page.getByLabel('Email *').fill('test@example.com');failEmail=true;await page.getByRole('button',{name:'Create account & save my report'}).click();await page.getByRole('alert').filter({hasText:'Email delivery unavailable'}).waitFor();failEmail=false;await page.getByRole('button',{name:'Create account & save my report'}).click();await page.getByRole('status').filter({hasText:'Check your email'}).waitFor();check('contextual account creation keeps only the minimum identity needed to save the report');
 ...
 await page.goto(callback);await page.getByRole('alert').filter({hasText:'Temporary claim error'}).waitFor();assert.ok(await page.evaluate(key=>localStorage.getItem(key),CLAIM_KEY));await page.getByRole('button',{name:'Save my DNA'}).click();await page.getByRole('heading',{name:'MAVERICK'}).waitFor();assert.equal(claims,2);assert.equal(await page.evaluate(key=>localStorage.getItem(key),CLAIM_KEY),null);assert.equal(await page.evaluate(key=>sessionStorage.getItem(key),EPHEMERAL_RESULT_KEY),null);assert.equal(authUser.user_metadata.first_name,'Mahdi');assert.equal(authUser.user_metadata.age,35);assert.equal(authUser.user_metadata.last_name,undefined);assert.equal(authUser.user_metadata.phone,undefined);check('magic-link callback retries claim safely, clears guest-only state, and retains minimal account personalization');

 await page.getByRole('link',{name:'DNA',exact:true}).click();await page.waitForURL(url=>url.pathname==='/dna');await page.getByRole('link',{name:'Retake assessment',exact:true}).click();await page.waitForURL(url=>url.pathname==='/dna/assessment'&&!url.searchParams.has('fresh'));await page.getByRole('button',{name:'Start as guest'}).waitFor();assert.equal(await page.getByRole('heading',{name:'MAVERICK'}).count(),0);check('mobile DNA navigation exposes an explicit retake that starts clean instead of bouncing back to the old result');
 await page.goto(ORIGIN+'/profile');await page.getByRole('heading',{name:'MAVERICK'}).waitFor();
 await page.getByRole('link',{name:'Profile',exact:true}).click();await page.waitForURL(url=>url.pathname==='/account');await page.getByRole('button',{name:'Sign out',exact:true}).click();await page.getByRole('heading',{name:'Save your DNA when it becomes useful.',exact:true}).waitFor();await page.getByRole('link',{name:'Create free account',exact:true}).waitFor();await page.getByRole('link',{name:'Sign in',exact:true}).waitFor();check('mobile account sign out clears personal profile view and separates account creation from sign in');
 ...
 await page.screenshot({path:path.join(artifacts,'assessment-v110-mobile.png'),fullPage:true});assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));assert.deepEqual(runtimeErrors,[]);check('personalized assessment, report, account-save, retake and goal-context flow have no browser runtime errors');
 await browser.close();
