...
(async()=>{
 ...
 await page.reload();await page.getByRole('heading',{name:'MAVERICK'}).waitFor();resultText=await page.locator('body').innerText();assert.match(resultText,/Mahdi/);assert.match(resultText,/DNA-only ETF comparisons/i);check('guest report survives a real page refresh for the current browser session');
