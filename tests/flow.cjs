...
(async()=>{
 ...
 await page.getByRole('link',{name:'See all DNA matches'}).click();await page.waitForURL(url=>url.pathname==='/match');await page.getByRole('heading',{name:'See how your DNA lines up with ETFs'}).waitFor();await page.getByText('VFV',{exact:true}).first().waitFor();const dnaOnlyText=await page.locator('body').innerText();assert.doesNotMatch(dnaOnlyText,/Sign in to see compatibility/);assert.match(dnaOnlyText,/DNA-only/i);assert.doesNotMatch(dnaOnlyText,/82\/100/);check('same-session guest sees DNA-only Match with global status overriding stale numeric row data');
