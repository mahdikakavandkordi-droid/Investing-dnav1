...
(async()=>{
 ...
 await page.getByRole('link',{name:/Retake assessment/}).click();await page.waitForURL(url=>url.pathname==='/dna/assessment'&&!url.searchParams.has('fresh'));await page.getByRole('button',{name:'Start as guest'}).waitFor();assert.equal(await page.getByRole('heading',{name:'MAVERICK'}).count(),0);check('explicit retake starts a clean assessment instead of bouncing back to the old result');
