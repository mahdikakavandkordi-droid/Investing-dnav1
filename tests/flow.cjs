...
(async()=>{
 ...
 const resultText=await page.locator('body').innerText();assert.match(resultText,/Mahdi/);assert.match(resultText,/Age/);assert.match(resultText,/Risk tolerance/);assert.match(resultText,/Financial capacity/);assert.match(resultText,/Your decision fingerprint/);assert.match(resultText,/cognitive & emotional bias signals/i);assert.match(resultText,/Worth watching/i);assert.match(resultText,/research output/i);
