...
(async()=>{
 ...
 check('submission retry reaches personalized ephemeral report and redacts stale numeric Match data before context');
 ...
 await page.goto(ORIGIN+'/dna');await page.waitForURL(url=>url.pathname==='/dna');await page.getByRole('link',{name:'View my current DNA',exact:true}).click();await page.waitForURL(url=>url.pathname==='/dna/result');await page.getByRole('heading',{name:/Mahdi, a clearer path for this goal\./}).waitFor();assert.match(await page.locator('body').innerText(),/MAVERICK/);assert.match(await page.locator('body').innerText(),/same DNA/i);assert.match(await page.locator('body').innerText(),/Principal protection/i);assert.match(await page.locator('body').innerText(),/DNA Match paused/i);check('applied report keeps identity and archetype while showing the full goal context that actually affects Match');
