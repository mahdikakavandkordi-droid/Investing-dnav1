...
(async()=>{
 ...
 await page.reload();await page.getByRole('heading',{name:'MAVERICK'}).waitFor();resultText=await page.locator('body').innerText();assert.match(resultText,/Mahdi/);assert.match(resultText,/DNA-only fund comparisons/i);assert.doesNotMatch(resultText,/82\/100/);check('guest report survives refresh without exposing a stale pre-context score');

 await page.getByRole('link',{name:'See all DNA matches'}).click();await page.waitForURL(url=>url.pathname==='/match');const matchHeading=page.locator('.match-dna-hero h1');await matchHeading.waitFor();assert.match(await matchHeading.innerText(),/See how your DNA lines up with funds|Your fund matches/);await page.getByText('VFV',{exact:true}).first().waitFor();const dnaOnlyText=await page.locator('body').innerText();assert.doesNotMatch(dnaOnlyText,/Sign in to see compatibility/);assert.match(dnaOnlyText,/DNA-only/i);assert.doesNotMatch(dnaOnlyText,/82\/100/);check('same-session guest sees DNA-only Match with global status overriding stale numeric row data');
 ...
 await page.waitForURL(url=>url.pathname==='/match');await page.getByRole('heading',{name:'Review this money before ranking funds'}).waitFor();assert.equal(contextSaves,1);assert.match(await page.locator('body').innerText(),/Matching paused/i);assert.doesNotMatch(await page.locator('body').innerText(),/82\/100/);assert.equal(await page.locator('.match-dna-card').count(),0);check('goal context preserves identity, saves all core constraints, and fully pauses stale ranking cards under review');

 await page.goto(ORIGIN+'/dna');await page.waitForURL(url=>url.pathname==='/dna');await page.getByRole('link',{name:'View my current DNA',exact:true}).click();await page.waitForURL(url=>url.pathname==='/dna/result');await page.getByRole('heading',{name:/Mahdi, a clearer path for this goal\./}).waitFor();const appliedText=await page.locator('body').innerText();assert.match(appliedText,/MAVERICK/);assert.match(appliedText,/same DNA/i);assert.match(appliedText,/Principal protection/i);assert.match(appliedText,/Part 3 · Asset mix scenarios/i);assert.match(appliedText,/Your Core Blueprint/i);assert.match(appliedText,/0%\s*Equity/i);assert.match(appliedText,/Constraint capped/i);assert.match(appliedText,/DNA Match paused/i);check('applied report keeps identity and archetype, adds a protection-constrained blueprint, and preserves the Match stop');
 ...
 await page.getByRole('link',{name:'Profile',exact:true}).click();await page.waitForURL(url=>url.pathname==='/account');await page.getByRole('button',{name:'Sign out',exact:true}).click();await page.getByRole('heading',{name:'Save your DNA when it becomes useful.',exact:true}).waitFor();await page.getByRole('link',{name:'Sign in or create account',exact:true}).click();await page.getByLabel('Email address').waitFor();check('mobile account sign out clears personal profile view');
