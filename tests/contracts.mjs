...
assert.doesNotMatch(homeAssetRailSource,/const ORDER=\[[^\]]*BOND/);
assert.doesNotMatch(homeAssetRailSource,/const ORDER=\[[^\]]*T_BILL/);
assert.doesNotMatch(homeAssetRailSource,/const ORDER=\[[^\]]*COMMERCIAL_PAPER/);
const instrumentModelSource=readFileSync(new URL('../lib/instrument-model.ts',import.meta.url),'utf8');
assert.match(instrumentModelSource,/PUBLIC_V1_ASSET_TYPES = \['ETF','MUTUAL_FUND','GIC'\]/);
assert.match(instrumentModelSource,/label:'Mutual Fund'/);
assert.match(instrumentModelSource,/matchEligible:true/);
const instrumentReadModelSource=readFileSync(new URL('../lib/instruments.ts',import.meta.url),'utf8');
assert.match(instrumentReadModelSource,/deposit_term_options\?:DepositTermOption\[\]/);
assert.match(instrumentReadModelSource,/gicTermRange/);
assert.match(instrumentReadModelSource,/gicRateRange/);
const gicMarketMigration=readFileSync(new URL('../supabase/migrations/20260921020500_v1_gic_market_families.sql',import.meta.url),'utf8');
assert.match(gicMarketMigration,/create table if not exists public\.investment_deposit_term_options/);
assert.match(gicMarketMigration,/RBC-GIC-NR/);
assert.match(gicMarketMigration,/TD-GIC-NR/);
assert.match(gicMarketMigration,/BMO-GIC-NR/);
assert.match(gicMarketMigration,/CIBC-GIC-BONUS/);
assert.match(gicMarketMigration,/SCOTIA-GIC-NR/);
assert.match(gicMarketMigration,/NBC-GIC-NR/);
assert.match(gicMarketMigration,/TNG-GIC-NR/);
assert.match(gicMarketMigration,/coalesce\(term_curve\.options,'\[\]'::jsonb\) deposit_term_options/);
assert.match(gicMarketMigration,/intentionally not inferred here/);
