...
server.stdout.on('data',d=>{if(d.toString().includes('Ready')&&!started){started=true;const test=spawn(process.execPath,[process.argv[2]||'tests/flow.cjs'],{env,stdio:'inherit'});test.on('exit',code=>{server.kill('SIGTERM');process.exitCode=code;});}});
