...
export default function Result(){
 ...
 return <section className="section"><div className="container narrow"><div className="card">{loading?<h1>Loading your DNA…</h1>:dna?<><DnaSummary dna={dna} report={report}/><p className="notice">{pending?'Your result is ready. Create a free account or sign in to keep it beyond this browser session.':'Your DNA is linked to your account.'}</p><div className="actions"><Link className="btn primary" href="/profile">{pending?'Save my DNA':'My profile'}</Link><Link className="btn" href="/explore">Explore investments</Link></div></>:<><h1>{error?'Could not load your DNA':'Your DNA starts here'}</h1>{error?<p role="alert">{error}</p>:<p>No completed assessment is available on this browser. Sign in to view a saved result.</p>}<div className="actions"><Link className="btn primary" href="/dna/assessment">Start / resume assessment</Link><Link className="btn" href="/profile">Sign in</Link>{error&&<button className="btn" onClick={()=>location.reload()}>Retry</button>}</div></>}</div></div></section>;
}
