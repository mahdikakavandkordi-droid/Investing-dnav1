...
export function DnaSummary({dna,report,personal}:{dna:DNA;report?:DNA|null;personal?:PersonalizationProfile|null}){
 ...
 return <div className="dna-report dna-report-v2">
  ...
  {context&&<AppliedDnaSection context={context} firstName={firstName} displayName={displayName}/>} 
...
function BehaviorCard({label,help,band,value}:{label:string;help:string;band:string;value:unknown}){const numeric=typeof value==='number'?value:0;const width=Math.max(0,Math.min(100,numeric));return <div className="behavior-card"><div className="behavior-card-top"><span>{label}</span><strong>{band}</strong></div><div className="scorebar"><div style={{width:`${width}%`}}/></div><p>{help}</p></div>}
...
function signalStatus(value:unknown){const n=typeof value==='number'?value:50;if(n<40)return {label:'Worth watching',className:'watch'};if(n<60)return {label:'Possible signal',className:'possible'};return {label:'Lower signal',className:''}}
...
function AppliedDnaSection({context,firstName,displayName}:{context:NonNullable<DNA['investment_context']>;firstName?:string;displayName:string}){
 ...
 const values=[['Goal',formatInvestmentContext('goal',context.goal)],['Time horizon',formatInvestmentContext('time_horizon',context.time_horizon)],['Amount',amount],['Liquidity need',formatInvestmentContext('liquidity_need',context.liquidity_need)]];
