...
function RiskMetric({label,value}:{label:string;value:unknown}){const {locale,pick}=useLocale();const available=typeof value==='number'&&Number.isFinite(value);return <div className="overview-risk-metric"><span>{label}</span><strong>{riskBand(value,locale)}</strong><small>{available?`${score(value)}/100`:pick('Not scored','Non noté')}</small></div>}
...
function PlainEnglishSection({summary,decisionText,pressureText}:{summary:string;decisionText:string;pressureText:string}){const {pick}=useLocale();return <section className="report-section report-story report-story-v2"><div className="eyebrow">{pick("Your profile in plain English","Votre profil en langage clair")}</div><h2>{pick("The longer read","Lecture détaillée")}</h2><p>{summary}</p><p>{decisionText}</p><p>{pressureText}</p></section>}
