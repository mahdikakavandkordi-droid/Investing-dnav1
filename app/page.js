...
export default function Home(){
  ...
  async function start(){
    ...
    try {
      ...
      setSession(d); 
      const q=await api("questionnaire",{session_token:d.session_token});
      setQuestions(q.questions || q);
      setScreen("quiz");
  ...
  async function save(qid,value){
    ...
    try { await api("save_answers",{session_token:session.session_token,answers:[{question_id:qid,answer_value:value}]}); }
    catch(e){setError(e.message)}
  }
  async function submit(){
    setBusy(true); setError("");
    try {
      const d=await api("submit",{session_token:session.session_token});
  ...
  if(screen==="quiz"){
    const q=questions[idx], pct=Math.round(((idx)/questions.length)*100), value=q && answers[q.question_id];
