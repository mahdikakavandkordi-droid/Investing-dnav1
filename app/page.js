...
export default function Home(){
  ...
  const Brand=({compact=false})=><img className={compact?"brand-logo compact":"brand-logo"} src="/logo.png" alt="Investing DNA"/>;
  ...
  if(screen==="quiz"){
    ...
    return <main className="shell"><section className="quiz">
      ...
      {isScale ? <div className="scale-wrap">
        ...
        {opts.map((o,i)=>{const ov=optionValue(o,i),label=optionLabel(o,i);return <button key={i} className={String(value)===String(ov)?"option selected":"option"} onClick={()=>save(q.question_id,ov)}><span className="radio">{String(value)===String(ov)?"✓":""}</span>{label}</button>})}
      </div>}
      {error&&<div className="error">{error}</div>}
