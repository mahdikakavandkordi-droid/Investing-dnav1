...
export default function Dashboard(){
 ...
 const dna=state?.dna;const context=dna?.investment_context;const matchStatus=state?.matches?.status;const closest:MatchItem|undefined=state?.matches?.top_matches?.[0]||state?.matches?.alternatives?.[0];const archetype=dna?.narrative?.archetype_name||dna?.narrative?.archetype||dna?.archetype;const contextReady=!!(context?.goal&&context?.time_horizon);
 return <main className="dashboard-page"><div className="container dashboard-shell">
  <header className="dashboard-hero"><div><div className="eyebrow">My Investor DNA</div><h1>Your research workspace</h1><p>Pick up where you left off without rebuilding your profile each visit.</p></div><Link className="dashboard-account" href="/profile"><span className="dashboard-avatar">{(user.email||'U').slice(0,1).toUpperCase()}</span><span><strong>Account</strong><small>{user.email}</small></span><span aria-hidden>›</span></Link></header>
  ...
  <section className="dashboard-main-grid"><article className="dashboard-panel dashboard-next"><div className="dashboard-panel-head"><div><div className="eyebrow">Next best step</div><h2>{nextTitle(!!dna,contextReady,matchStatus)}</h2></div><span className="dashboard-step">01</span></div><p>{nextCopy(!!dna,contextReady,matchStatus)}</p><div className="actions compact"><Link className="btn primary" href={nextHref(!!dna,contextReady,matchStatus)}>{nextCta(!!dna,contextReady,matchStatus)}</Link>{dna&&<Link className="btn" href="/explore">Explore investments</Link>}</div></article>
   <article className="dashboard-panel"><div className="dashboard-panel-head"><div><div className="eyebrow">DNA Match</div><h2>{closest?closest.symbol:'ETF compatibility'}</h2></div>{closest?.match_score!=null&&matchStatus==='available'?<div className="dashboard-match-score"><strong>{Math.round(closest.match_score)}</strong><small>/100</small></div>:<span className="pill">{matchLabel(matchStatus)}</span>}</div><p>{closest?(closest.explanation?.summary||closest.fit_label||'Open Match to review why this ETF aligns with your current inputs.'):'Your Match view appears here after your DNA and required context are ready.'}</p><Link className="dashboard-text-link" href="/match">Open DNA Match →</Link></article></section>
  <section className="dashboard-panel dashboard-saved"><div className="dashboard-panel-head"><div><div className="eyebrow">Saved investments</div><h2>Your research queue</h2></div><Link className="dashboard-text-link" href="/watchlist">View all</Link></div>{items.length?<div className="dashboard-saved-list">{items.slice(0,4).map(item=><Link href={'/investment/'+item.investment_id} key={item.investment_id} className="dashboard-saved-row"><span className="dashboard-symbol">{item.symbol||'—'}</span><span><strong>{item.name||item.symbol}</strong><small>Saved for later research</small></span><span aria-hidden>›</span></Link>)}</div>:<div className="dashboard-inline-empty"><p>No saved investments yet.</p><Link className="btn" href="/explore">Explore investments</Link></div>}</section>
