...
function GuestSaveCard({personal,sending,message,onSubmit}:{personal:PersonalizationProfile|null;sending:boolean;message:string;onSubmit:(email:string,profile:PersonalizationProfile)=>Promise<void>}){
 ...
 return <>
  ...
  {open&&<div className="save-modal-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)setOpen(false)}}><section className="save-modal" role="dialog" aria-modal="true" aria-labelledby="save-report-title"><div className="save-modal-head"><div><div className="eyebrow">{pick("Free · passwordless","Gratuit · sans mot de passe")}</div><h2 id="save-report-title">{pick("Save your Investor DNA","Enregistrer votre Investor DNA")}</h2><p>{pick("Email is required for secure access. Last name and phone are optional.","Le courriel est requis pour un accès sécurisé. Le nom de famille et le téléphone sont facultatifs.")}</p></div><button className="save-modal-close" aria-label={pick("Close","Fermer")} onClick={()=>setOpen(false)}>×</button></div><form onSubmit={submit}><div className="save-modal-fields"><label><span>Email *</span><input type="email" autoComplete="email" required value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com"/></label><label><span>{pick("First name *","Prénom *")}</span><input autoComplete="given-name" required value={firstName} onChange={e=>setFirstName(e.target.value)}/></label><label><span>{pick("Last name (optional)","Nom de famille (facultatif)")}</span><input autoComplete="family-name" value={lastName} onChange={e=>setLastName(e.target.value)}/></label><label><span>{pick("Phone (optional)","Téléphone (facultatif)")}</span><input type="tel" autoComplete="tel" value={phone} onChange={e=>setPhone(e.target.value)} placeholder="+1 …"/></label></div><button className="btn primary" disabled={sending||!email.trim()||!firstName.trim()}>{sending?pick('Sending secure link…','Envoi du lien sécurisé…'):pick('Save & email my report','Enregistrer et envoyer mon rapport')}</button><p className="save-modal-fine">{pick("The email contains a secure sign-in link, not an attachment. For this guest report, open the first save link in this same browser; after the report is attached to your account, it can be reopened from other devices.","Le courriel contient un lien de connexion sécurisé, pas une pièce jointe. Pour ce rapport invité, ouvrez le premier lien d’enregistrement dans ce même navigateur; une fois le rapport associé à votre compte, vous pourrez le rouvrir sur d’autres appareils.")}</p>{message&&<p className="notice" role="status">{message}</p>}</form></section></div>}
 </>;
}
