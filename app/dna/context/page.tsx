...
export default function InvestmentContext(){
 ...
 async function save(){if(!hasAssessment||!coreReady)return;setBusy(true);setError('');try{
  const context={first_name:form.first_name.trim()||null,age:form.age?Number(form.age):null,amount_to_invest:form.amount_to_invest?Number(form.amount_to_invest):null,goal:form.goal,time_horizon:form.time_horizon,liquidity_need:form.liquidity_need,principal_required:form.principal_required};
 ...
 return <ContextShell><div className="context-card"><div className="eyebrow">Your DNA in action</div><h1>{editing?'Review this investment goal.':'Let’s put your DNA into action.'}</h1><p className="context-lede">Your DNA describes you. These answers describe this particular pool of money and never rewrite your personal Investor DNA.</p><div className="context-identity"><div><small>Your personal profile</small><strong>{personal?.first_name||form.first_name||'Investor'}{personal?.age?` · Age ${personal.age}`:form.age?` · Age ${form.age}`:''}</strong></div><span>Name and age stay with your report.</span></div><CoreContextFields form={form} update={update}/><label className="context-field context-amount"><span>Amount you are considering (CAD) · optional</span><input type="number" min="0" value={form.amount_to_invest} onChange={event=>update('amount_to_invest',event.target.value)} placeholder="25,000"/></label>{error&&<p role="alert" className="notice">{error}</p>}<div className="context-actions"><button className="btn primary" disabled={busy||!coreReady} onClick={()=>void save()}>{busy?'Saving…':editing?'Update this goal':'See my personalized insights'}</button><Link className="btn" href={resolveCancelTarget()}>{editing?'Cancel':'Skip for now'}</Link></div>{!coreReady&&<p className="muted fine">Choose the four core goal answers above to create a context-aware Match. The amount can stay blank.</p>}<p className="muted fine">Context improves compatibility signals; it does not turn them into investment advice.</p></div></ContextShell>;
}
