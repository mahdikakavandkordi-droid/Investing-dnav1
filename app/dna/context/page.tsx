...
import type {Draft,MatchPayload,Submission} from "@/lib/dna";
...
/**
 * Investment-context step kept separate from Investor DNA scoring.
 * Context describes this pool of money and may refine Match; it must not be
 * folded back into the underlying risk-tolerance score.
 */
export default function InvestmentContext(){
 ...
 async function save(){
  ...
  try{
   ...
   const data=await pilot<ContextSaveResponse>('save_context',{...draft.session,context});
   const next:Draft={
    ...draft,
    result:draft.result
     ? {
 ...
 if(loading)return <ContextShell><div className="context-card"><h1>Loading…</h1></div></ContextShell>;
 if(!draft?.result)return <ExpiredContext/>;

 return <ContextShell>
  <div className="context-card">
   <div className="eyebrow">Investment context</div>
   <h1>Tell us what this money is for.</h1>
   <p className="context-lede">Your DNA describes you. These details describe this investment decision, so they stay separate from your risk-tolerance score.</p>
   ...
   <div className="context-actions">
    <button className="btn primary" disabled={busy} onClick={()=>void save()}>{busy?'Saving…':'Use this context'}</button>
    <Link className="btn" href="/dna/result">Skip for now</Link>
   </div>
...
function ExpiredContext(){
 return <ContextShell>
  <div className="context-card">
   <h1>This guest report is no longer available.</h1>
   <p>Guest DNA reports disappear after refresh or leaving the flow unless they are saved to an account.</p>
...
function CoreContextFields({form,update}:{form:ContextForm;update:<K extends keyof ContextForm>(key:K,value:ContextForm[K])=>void}){
 return <div className="context-fields">
  ...
  <label className="context-field">
   <span>When might you need this money?</span>
...
function OptionalPersonalization({form,update}:{form:ContextForm;update:<K extends keyof ContextForm>(key:K,value:ContextForm[K])=>void}){
 return <div className="context-optional">
  <div>
   ...
   <p>These details make the report more personal. They do not change your Investor DNA score.</p>
  </div>
