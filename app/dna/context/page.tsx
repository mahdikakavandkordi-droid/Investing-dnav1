...
import {readDraft,writeDraft,Draft,MatchPayload} from "@/lib/dna";
...
export default function InvestmentContext(){
  ...
  async function save(){
    ...
    try{
      ...
      const data=await pilot<{report?:Draft['result'] extends infer _T?any:any;match?:MatchPayload}>('save_context',{...draft.session,context});
  ...
  if(!draft?.result)return <main className="context-page"><div className="container context-container"><div className="context-card"><h1>Complete your Investor DNA first.</h1><Link className="btn primary" href="/dna/assessment">Start assessment</Link></div></div></main>;
