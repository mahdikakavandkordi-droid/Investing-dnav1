...
import {Fund,InvestmentDna,OfficialFundFacts,ResearchContext,fund,investmentDna,officialFundFacts,researchContext,formatMetric,validId} from '@/lib/investments';
...
export default function Detail(){
 ...
 const [item,setItem]=useState<Fund|null>(null),[dna,setDna]=useState<InvestmentDna|null>(null),[facts,setFacts]=useState<OfficialFundFacts|null>(null),[research,setResearch]=useState<ResearchContext|null>(null),[loading,setLoading]=useState(true),[error,setError]=useState(''),[retry,setRetry]=useState(0);
 ...
 Promise.all([fund(id),investmentDna(id).catch(()=>null),officialFundFacts(id).catch(()=>null),researchContext(id).catch(()=>null)]).then(([fundData,dnaData,factsData,researchData])=>{if(active){setItem(fundData);setDna(dnaData);setFacts(factsData);setResearch(researchData)}}).catch(e=>{if(active)setError(e.message)}).finally(()=>{if(active)setLoading(false)});
 ...
 <div className="grid3"><div className="card"><small>Price {item.currency?`(${item.currency})`:''}</small><div className="metric-value">{formatMetric(item.price)}</div></div><div className="card"><small>1-year return</small><div className="metric-value">{formatMetric(item.return_1y_pct,'%')}</div></div><div className="card"><small>MER</small><div className="metric-value">{formatMetric(item.mer_pct,'%')}</div></div></div>
 <p className="muted fine">{item.data_status==='identity_only'?'Fund profile available; live price and performance data are not available.':`Latest displayed research input as of ${item.metrics_as_of_date||'an unreported date'}.`} Missing figures are not shown as zero.</p>
