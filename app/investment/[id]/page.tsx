...
import {Fund,fund,formatMetric,validId} from '@/lib/investments';
import {FundConnection} from '@/components/FundConnection';
export default function Detail(){
 const {id}=useParams<{id:string}>();const [item,setItem]=useState<Fund|null>(null),[loading,setLoading]=useState(true),[error,setError]=useState(''),[retry,setRetry]=useState(0);
 useEffect(()=>{let active=true;setItem(null);setLoading(true);setError('');
 if(!validId(id)){setLoading(false);setError('This fund link is invalid.');return;}
 fund(id).then(d=>{if(active)setItem(d)}).catch(e=>{if(active)setError(e.message)}).finally(()=>{if(active)setLoading(false)});
