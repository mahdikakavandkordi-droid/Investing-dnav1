...
import {getInstrument,instrumentDisplayName,instrumentWatchlist,saveInstrument} from '@/lib/instruments';
...
export default function Profile(){
 ...
 async function saveIncomingInvestment(){if(!intent||!user||busy)return;setBusy(true);setError('');try{const result=await saveInstrument(intent.id);if(!result.item)throw new Error('Saving was not confirmed.');setMessage(`${intent.symbol||instrumentDisplayName(intent)} is saved to your watchlist.`);void trackProductEvent('watchlist_saved',{investment_id:intent.id,metadata:{source:'profile_intent'}});setRetry(v=>v+1)}catch(e){setError(e instanceof Error?e.message:'Unable to save this investment.')}finally{setBusy(false)}}
 async function signOut(){if(!supabase||busy)return;setBusy(true);try{const currentUserId=user?.id;const {error:authError}=await supabase.auth.signOut({scope:'local'});if(authError)throw authError;clearReturningWorkspaceCache(currentUserId);clearDraft();restoredTracked.current=false;setCallbackMode(null);setPending(null);setState(null);setItems([]);setMessage('You have signed out.')}catch(e){setError(e instanceof Error?e.message:'Unable to sign out.')}finally{setBusy(false)}}
...
function SignedInDashboard({email,personal,intent,intentSaved,pending,state,items,listError,closest,busy,onSaveIntent,onClaim,onRetryList,onSignOut}:{email:string;personal:PersonalizationProfile|null;intent:Instrument|null;intentSaved:boolean;pending:ClaimTicket|null;state:AppState|null;items:SavedInstrument[];listError:string;closest?:MatchItem;busy:boolean;onSaveIntent:()=>void;onClaim:()=>void;onRetryList:()=>void;onSignOut:()=>void}){
 ...
 {intent&&<div className="dashboard-intent"><div><span className="dashboard-kicker">Save intent</span><strong>{intent.symbol||instrumentDisplayName(intent)}</strong><p>{intentSaved?'Already in your watchlist.':'Your research intent is still intact.'}</p></div><div className="actions compact">{!intentSaved&&<button className="btn primary" disabled={busy} onClick={onSaveIntent}>Save to watchlist</button>}<Link className="btn" href={'/investment/'+intent.id}>Open research</Link></div></div>}
 ...
 return <div className="mobile-dashboard-home">
  ...
  <section className="mobile-home-section">
   ...
   <div className="mobile-action-list">
    ...
    <Link href="/explore"><span className="mobile-action-icon">⌕</span><span><strong>Explore investments</strong><small>ETFs, Mutual Funds and GICs</small></span><b>›</b></Link>
  ...
  {items.length>0&&<section className="mobile-home-section mobile-saved-preview">
   ...
   <div className="mobile-saved-list">{items.slice(0,3).map(item=><Link key={item.investment_id} href={'/investment/'+item.investment_id}><span className="mobile-saved-symbol">{item.symbol||'Fund'}</span><span><strong>{item.display_name||item.name}</strong><small>Saved research</small></span><b>›</b></Link>)}</div>
  </section>}
 </div>;
}

function MatchDashboardCopy({matchStatus,closest,hasDna}:{matchStatus?:string;closest?:MatchItem;hasDna:boolean}){if(!hasDna)return <><p>Complete Investor DNA before using personal fund compatibility.</p><Link className="btn primary" href="/dna/assessment">Start assessment</Link></>;if(matchStatus==='available'&&closest)return <><p>{closest.name||closest.symbol} is currently the closest research compatibility in your eligible fund set. This is not a recommendation.</p><Link className="btn primary" href="/match">Open DNA Match</Link></>;if(matchStatus==='context_required')return <><p>Numeric scores stay hidden until this money has a goal, horizon, access need and principal-protection choice.</p><Link className="btn primary" href="/dna/context?returnTo=/profile">Add goal context</Link></>;if(matchStatus==='review_required')return <><p>A safety or data-review gate is active, so ranked numeric results are paused.</p><Link className="btn primary" href="/match">Review Match status</Link></>;return <><p>Your DNA is connected. Open Match to continue compatibility research.</p><Link className="btn primary" href="/match">Open DNA Match</Link></>}
function SavedInvestments({items,listError,onRetry}:{items:SavedInstrument[];listError:string;onRetry:()=>void}){if(listError)return <div className="dashboard-inline-error"><p>{listError}</p><button className="btn" onClick={onRetry}>Retry watchlist</button></div>;if(!items.length)return <><p>No saved investments yet. Save from any investment page and it will stay here.</p><Link className="btn" href="/explore">Explore investments</Link></>;return <div className="dashboard-saved-list">{items.slice(0,3).map(item=><Link key={item.investment_id} href={'/investment/'+item.investment_id}><strong>{item.symbol||item.display_name||item.name}</strong><span>{item.display_name||item.name}</span></Link>)}</div>}
