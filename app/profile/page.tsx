...
export default function Profile(){
 ...
 const [state,setState]=useState<AppState|null>(null),[pending,setPending]=useState<Draft|null>(null),[items,setItems]=useState<WatchItem[]>([]);
 ...
 const [retry,setRetry]=useState(0),[mode,setMode]=useState<'signin'|'signup'>('signin'),[intent,setIntent]=useState<Fund|null>(null),[intentId,setIntentId]=useState<string|null>(null);
 useEffect(()=>{
  const url=new URL(location.href);setMode(url.searchParams.get('mode')==='signup'?'signup':'signin');const id=url.searchParams.get('investment');
 ...
 useEffect(()=>{let active=true;setState(null);setItems([]);setListError('');setLoading(true);
  const draft=readDraft(user?.id||null);setPending(draft?.result&&!draft.result.account_linked?draft:null);
  if(authLoading)return;
  if(!user){setLoading(false);return;}
  // These are independent: a DNA error must not hide saved funds.
 ...
 async function signIn(e:React.FormEvent){e.preventDefault();if(!supabase||busy)return;setBusy(true);setError('');setMessage('');
  try{const redirect=new URL('/profile',location.origin);if(intentId)redirect.searchParams.set('investment',intentId);
   ...
   setMessage(mode==='signup'?'Check your email to confirm your free account. Your fund will be waiting here.':'Check your email for a sign-in link. Open it in this browser to keep your assessment available.');
 ...
 const intentSaved=items.some(x=>x.investment_id===intent?.id);
 ...
 {pending&&<div className="notice"><h2>Your new DNA is ready to save</h2><p>Save this assessment to {user.email}. It will become your current DNA.</p><button className="btn primary" disabled={busy} onClick={claim}>{busy?'Saving…':'Save this DNA to my account'}</button></div>}
 ...
 {pending&&<p className="notice">Your completed DNA is ready on this browser. You can save it after signing in.</p>}
 ...
 <p className="fine muted">No password needed. Registration is optional; browsing funds and taking the assessment remain open to guests. Keep the email link in this browser if you want to save a guest DNA result.</p><Link href={intentId?'/investment/'+intentId:pending?'/dna/result':'/dna/assessment'}>{intentId?'Back to fund':pending?'Back to my result':'Continue without an account'} →</Link>
 </>}
