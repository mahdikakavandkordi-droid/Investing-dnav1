...
export default function Profile(){
 ...
 async function sendMagicLink(event:FormEvent){event.preventDefault();if(!supabase||busy||cooldown>0)return;const address=email.trim();if(!address)return;setBusy(true);setError('');setMessage('');try{const redirect=new URL('/profile',location.origin);if(intentId)redirect.searchParams.set('investment',intentId);if(pending)redirect.searchParams.set('save','dna');const {error:authError}=await supabase.auth.signInWithOtp({email:address,options:{shouldCreateUser:true,emailRedirectTo:redirect.toString(),data:pending?.personalization||undefined}});if(authError)throw authError;setSentTo(address);setCooldown(60);setMessage('Check your email for the secure sign-in link.')}catch(e){const raw=e instanceof Error?e.message:'Unable to send an email link.';setError(friendlyAuthError(raw));if(/rate limit|too many/i.test(raw))setCooldown(60)}finally{setBusy(false)}}
...
function SignedOutCard({email,sentTo,cooldown,busy,message,pending,onEmail,onSubmit}:{email:string;sentTo:string;cooldown:number;busy:boolean;message:string;pending:ClaimTicket|null;onEmail:(value:string)=>void;onSubmit:(event:FormEvent)=>void}){return <div className="card auth-card"><div className="eyebrow">My Investor DNA</div><h1>{pending?'Save your Investor DNA':'Create your free account or sign in'}</h1>{sentTo?<div className="auth-sent"><h2>Check your email</h2><p>We sent a secure sign-in link to <strong>{sentTo}</strong>.</p></div>:<form onSubmit={onSubmit}><p>{pending?'Use your email to attach the completed report to your dashboard.':'Use a secure email link — no password required.'}</p><label className="context-field"><span>Email address</span><input type="email" autoComplete="email" required value={email} onChange={e=>onEmail(e.target.value)} placeholder="you@example.com"/></label><button className="btn primary" disabled={busy||cooldown>0}>{busy?'Sending…':cooldown>0?`Try again in ${cooldown}s`:'Email me a secure link'}</button></form>}{message&&<p className="notice" role="status">{message}</p>}</div>}
