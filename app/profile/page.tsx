...
import {Instrument,SavedInstrument,getInstrument,instrumentWatchlist,saveInstrument} from '@/lib/instruments';
export default function Profile(){
 const {user,loading:authLoading}=useAccount();
 const [state,setState]=useState<AppState|null>(null),[pending,setPending]=useState<ClaimTicket|null>(null),[items,setItems]=useState<SavedInstrument[]>([]);
 ...
 const [retry,setRetry]=useState(0),[mode,setMode]=useState<'signin'|'signup'>('signin'),[intent,setIntent]=useState<Instrument|null>(null),[intentId,setIntentId]=useState<string|null>(null),[autoSave,setAutoSave]=useState(false),[canReturnToResult,setCanReturnToResult]=useState(false);
 useEffect(()=>{
  ...
  setIntentId(id);let active=true;getInstrument(id).then(d=>{if(active)setIntent(d)}).catch(()=>{if(active)setError('Could not load the investment you wanted to save. You can return to Explore.')});return ()=>{active=false;};
 },[]);
 useEffect(()=>{let active=true;setState(null);setItems([]);setListError('');setLoading(true);
  ...
  instrumentWatchlist().then(d=>{if(active)setItems(d.items)}).catch(e=>{if(active)setListError(e.message)});
  return ()=>{active=false;};
 ...
 async function claim(){if(!pending||!user||busy)return;setBusy(true);setError('');setMessage('');
  try{const result=await pilot<{claimed:boolean}>('claim_assessment',pending.session);if(!result.claimed)throw new Error('The service has not confirmed saving your DNA.');clearDraft();setPending(null);setCanReturnToResult(false);setMessage('Your Investing DNA is now saved to your account.');setRetry(n=>n+1)}catch(e){setError(e instanceof Error?e.message:'Unable to save this DNA right now. Your claim link remains available for a limited time.')}finally{setBusy(false)}}
 async function saveIntent(){if(!intent||!user||busy)return;setBusy(true);setError('');
  try{const result=await saveInstrument(intent.id);if(!result.item)throw new Error('Saving was not confirmed.');setMessage(`${intent.symbol||intent.name} is saved to your watchlist.`);setRetry(n=>n+1)}catch(e){setError(e instanceof Error?e.message:'Unable to save this investment.')}finally{setBusy(false)}}
 ...
 {intent&&<div className="notice"><h2>{intent.name}</h2><p>{intentSaved?'This investment is in your watchlist.':`Save this investment to ${user.email} so you can return to it later.`}</p><div className="actions">{!intentSaved&&<button className="btn primary" disabled={busy} onClick={saveIntent}>Save this investment to my watchlist</button>}<Link className="btn" href={'/investment/'+intent.id}>Back to investment</Link></div></div>}
 ...
 {state?.dna&&!pending&&<div className="profile-next"><div className="eyebrow">Continue where you left off</div>{matchStatus==='available'&&closest?<><h2>Your closest current ETF comparison: {closest.symbol}</h2><p>{closest.name||closest.symbol} is currently {closest.match_score==null?'under review':`${Math.round(closest.match_score)}/100`} on compatibility. Re-open the explanation or compare it with other eligible ETFs.</p><div className="actions"><Link className="btn primary" href="/match">Open my matches</Link><Link className="btn" href="/screener">Screen ETFs with my DNA</Link></div></>:matchStatus==='context_required'?<><h2>Your DNA is saved; your investment context is the missing piece</h2><p>Add the real goal and withdrawal horizon for this money so the product can move from DNA-only comparison to context-aware ETF matching.</p><Link className="btn primary" href="/dna/context">Add investment context</Link></>:matchStatus==='no_suitable_options'?<><h2>No current ETF passes all of your fit limits</h2><p>The platform kept the no-match state instead of forcing a recommendation. Open Match to see which constraints are creating the gap.</p><Link className="btn primary" href="/match">Understand the no-match result</Link></>:matchStatus==='review_required'?<><h2>Your ETF match is paused for review</h2><p>A safety or data-review gate is active. Open Match before treating any ETF as personally compatible.</p><Link className="btn primary" href="/match">Review match status</Link></>:<><h2>Your research workspace is ready</h2><p>You have {items.length} saved investment{items.length===1?'':'s'}. Continue with your watchlist, Explore the cross-asset universe, or revisit ETF matches.</p><div className="actions"><Link className="btn primary" href="/watchlist">Open my watchlist</Link><Link className="btn" href="/explore">Explore investments</Link></div></>}</div>}
 <section className="section compact"><h2>My saved investments</h2>{listError?<div role="alert"><p>{listError}</p><button className="btn" onClick={()=>setRetry(n=>n+1)}>Reload saved investments</button></div>:items.length?<><p>{items.length} investment{items.length===1?'':'s'} in your watchlist</p>{items.slice(0,3).map(x=><p key={x.investment_id}><Link href={'/investment/'+x.investment_id}>{x.symbol?`${x.symbol} — `:''}{x.name} →</Link></p>)}</>:<p>No saved investments yet. Open an investment and save it to keep it here.</p>}<div className="actions"><Link className="btn" href="/watchlist">My watchlist</Link><Link className="btn" href="/explore">Explore investments</Link></div></section>
 {state?.dna?<><div className="section compact"><DnaSummary dna={state.dna} report={state.report}/></div><div className="actions"><Link className="btn" href="/dna/result">View my DNA</Link><Link className="btn" href="/match">My ETF matches</Link></div></>:state&&!pending?<><h2>Connect your DNA</h2><p>Your account can keep saved investments now. Complete and save your assessment to add personal ETF compatibility.</p><Link className="btn primary" href="/dna/assessment">Start Investing DNA assessment</Link></>:null}
 ...
 <p className="muted">{mode==='signup'?'Save investment research and your DNA in one place. You can return from another device after signing in.':'Sign in with an email link to return to your saved investments and Investing DNA.'}</p>
 ...
 <p className="fine muted">No password needed. If you have an unsaved DNA result, open the email link in this browser so we can attach that assessment to your account.</p>{canReturnToResult?<Link href="/dna/result">Back to my one-time report →</Link>:<Link href={intentId?'/investment/'+intentId:'/dna/assessment'}>{intentId?'Back to investment':'Continue without an account'} →</Link>}
 </>}
