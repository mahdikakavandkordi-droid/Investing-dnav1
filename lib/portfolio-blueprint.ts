...
function cashFloor(context:InvestmentContextProfile){
  let floor=5;
...
function isGrowthBlocked(context:InvestmentContextProfile){
  return context.principal_required==='yes'||context.goal==='emergency_reserve'||context.time_horizon==='lt_1y';
}

export function buildPortfolioBlueprint(dna:DNA,context?:InvestmentContextProfile|null):PortfolioBlueprint|null{
  ...
  // Financial capacity is a ceiling, not a bonus. A user who is willing to take
  // more risk than their current finances can absorb should not receive a more
  // aggressive core allocation because of willingness alone.
