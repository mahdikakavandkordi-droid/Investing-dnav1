...
declare
  ...
  v_pref_fit numeric; v_cap_guard numeric; v_risk_fit numeric; v_exposure_fit numeric; v_goal_fit numeric; v_horizon_fit numeric; v_access_fit numeric; v_div_fit numeric; v_complexity_fit numeric; v_total numeric;
  ...
  v_model text:='investment-dna-match-v4.1';
begin
  ...
  for d in select * from public.v_investment_dna_v1 where data_quality_status='verified' and official_risk_rating is not null order by symbol loop
    ...
    v_pref_fit:=case v_rt_level
      ...
      else case v_official when 'Low' then 55 when 'Low to Medium' then 75 when 'Medium' then 95 when 'Medium to High' then 95 else 85 end end;
    v_cap_guard:=case v_rc_level
      ...
      else case v_official when 'High' then 90 else 100 end end;
    v_risk_fit:=v_pref_fit*.80+v_cap_guard*.20;
    -- Proprietary market-exposure signal is deliberately separate from the official issuer risk rating.
    v_exposure_fit:=case v_rt_level when 'low' then greatest(0,100-greatest(0,v_eq-50)*1.50) when 'moderate' then greatest(0,100-greatest(0,v_eq-70)*.75) else 100 end;
    if c.id is null or v_goal is null then v_goal_fit:=70;
    else v_goal_fit:=case when v_goal='growth' then v_growth when v_goal='retirement' then v_growth*.65+v_stability*.35 when v_goal='income' then v_income when v_goal in ('preservation','wealth_preservation','emergency_reserve') then v_stability when v_goal in ('house_purchase','education','major_purchase') then v_stability*.80+(100-v_eq)*.20 else 70 end; end if;
    ...
    if v_exposure_fit<70 then v_watchouts:=v_watchouts||jsonb_build_array('Its equity exposure may create more market movement than your answers suggest you would be comfortable with.'); end if;
...
declare v_limit int:=greatest(1,least(coalesce(p_limit,10),25)); v_model text:='investment-dna-match-v4.1'; v_count int; v_top jsonb; v_alt jsonb; v_consider jsonb; v_mismatch jsonb;
begin
