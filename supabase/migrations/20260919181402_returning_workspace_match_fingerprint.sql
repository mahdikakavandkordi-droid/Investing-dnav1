-- Refine return-loop semantics: a regenerated Match run is not itself a user-visible
-- change. Compare the canonical Match input fingerprint so "Match updated" means
-- the underlying scoring inputs changed.

alter table public.investor_workspace_state
  add column if not exists last_match_input_fingerprint text;
