-- Harden the public market-data read boundary and cover the new retention FK.
-- The exposed browser wrapper stays SECURITY INVOKER while the implementation
-- remains in the non-exposed private schema, matching the rest of the app RPC pattern.

create or replace function investor_private.market_data_status(p_investment_ids uuid[])
returns table(
  investment_id uuid,
...
revoke all on function investor_private.market_data_status(uuid[]) from public;
grant execute on function investor_private.market_data_status(uuid[]) to anon,authenticated,service_role;
...
set search_path=''
as $$
  select * from investor_private.market_data_status(p_investment_ids);
$$;
...
create index if not exists investor_workspace_item_state_investment_idx
  on public.investor_workspace_item_state(investment_id);
