create or replace function public.calculate_investment_match_v5(p_assessment_id uuid)
returns jsonb
language plpgsql
...
declare
  ...
  v_goal_fit numeric; v_horizon_fit numeric; v_access_fit numeric; v_total numeric;
  ...
  v_model text:='investment-dna-match-v5.0';
...
begin
  ...
  select * into c
  from public.investment_context
  where assessment_id=p_assessment_id
  ...
  for d in
    select *
    from public.v_investment_dna_v1
    where data_quality_status='verified' and official_risk_rating is not null
    order by symbol
  loop
    ...
    -- Official issuer risk compatibility. Risk capacity is a guardrail, not a target.
    ...
    -- Market exposure compatibility uses strategic equity allocation, separately from issuer risk.
    ...
    -- Goal fit uses only the transparent v1.1 Investment DNA role signals.
    ...
    -- Horizon fit is derived directly from user horizon and strategic equity exposure.
    if v_horizon is null then
      v_horizon_fit:=70;
    else
      v_horizon_equity_ceiling:=case
        when v_horizon<=12 then 20
        when v_horizon<=30 then 40
        when v_horizon<=48 then 60
        when v_horizon<=84 then 80
        else 100 end;
      if v_eq<=v_horizon_equity_ceiling then
        v_horizon_fit:=100;
      else
        v_horizon_fit:=greatest(0,100-(v_eq-v_horizon_equity_ceiling)*2);
      end if;
    end if;

    -- Liquidity/access fit uses the stability signal plus strategic equity exposure.
    ...
    -- v5 uses only inputs we can clearly explain to the user.
    v_total:=v_risk_fit*.35+v_exposure_fit*.20+v_goal_fit*.20+v_horizon_fit*.15+v_access_fit*.10;

    -- Hard guardrails for material mismatches.
    ...
    if v_horizon_fit>=90 then v_strengths:=v_strengths||jsonb_build_array('Its strategic market exposure is compatible with the time horizon you selected.'); end if;
    ...
    if v_goal_fit<60 and c.id is not null then v_watchouts:=v_watchouts||jsonb_build_array('The role this fund is built to play does not closely match the goal you selected.'); end if;
    if v_horizon_fit<70 then v_watchouts:=v_watchouts||jsonb_build_array('Its level of equity exposure may be high for the time horizon you selected.'); end if;
    if v_access_fit<60 then v_watchouts:=v_watchouts||jsonb_build_array('Your need for access to this money may conflict with this fund’s stability profile.'); end if;
    ...
    insert into public.investment_match_results(
      assessment_id,investment_id,model_version,match_score,risk_tolerance_fit,risk_capacity_fit,risk_band,rationale,explanation
    ) values(
      p_assessment_id,d.investment_id,v_model,round(v_total,2),round(v_pref_fit,2),round(v_cap_guard,2),v_official,
      jsonb_build_object(
        'engine',v_model,
        'signal_type','official-risk-plus-transparent-investment-dna-compatibility',
        'not_investment_advice',true,
        'weights',jsonb_build_object('official_risk_compatibility',35,'market_exposure_fit',20,'goal_role',20,'horizon_exposure',15,'access_stability',10),
    ...
    v_json:=v_json||jsonb_build_array(jsonb_build_object(
      'investment_id',d.investment_id,'symbol',d.symbol,'name',d.name,'match_score',round(v_total,2),'fit_label',v_fit,'official_risk_rating',v_official,'strengths',v_strengths,'watchouts',v_watchouts
    ));
    v_count:=v_count+1;
  end loop;
  ...
  return jsonb_build_object(
    ...
    'count',v_count,
    'context_applied',c.id is not null,
    'results',(select coalesce(jsonb_agg(x order by (x->>'match_score')::numeric desc),'[]'::jsonb) from jsonb_array_elements(v_json)x)
  );
end;
$function$;
...
set search_path to 'public','extensions'
as $function$
  select public.calculate_investment_match_v5(p_assessment_id);
$function$;