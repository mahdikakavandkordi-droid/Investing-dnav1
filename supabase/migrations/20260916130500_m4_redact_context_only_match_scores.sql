-- M4 trust/UI contract: a DNA-only comparison is not a personalized Match score.
--
-- The v7 engine may retain its internal comparison value in the persisted run
-- for research ordering/reproducibility, but current_match() redacts the public
-- overall match_score while investment context is incomplete. Component-level
-- research signals remain explainable; the consumer-facing /100 Match score
-- appears only after the money goal/context is complete.
...
begin
  ...
  return jsonb_set(
    jsonb_set(v_payload,'{results}',v_results,true),
    '{consider}',v_consider,true
