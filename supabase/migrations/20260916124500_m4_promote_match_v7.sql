-- M4: promote goal-aware Match v7 after side-by-side stress acceptance.
--
-- v6 remains as a historical/reproducible implementation but is no longer the
-- canonical current Match. The temporary v7 candidate wrapper is removed so
-- there is only one current read path: investor_private.current_match().
...
-- The candidate-only wrapper served its purpose; callers must use current_match.
drop function if exists investor_private.current_match_v7_candidate(uuid);

-- Versioned engines remain server-side only. v6 is retained for historical
-- reproducibility; v7 is the canonical current engine.
