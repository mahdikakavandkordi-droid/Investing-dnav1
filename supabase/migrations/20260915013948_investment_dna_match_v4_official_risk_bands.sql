...
declare
  ...
  v_rt_level text; v_rc_level text;
  v_horizon int:=null; v_goal text:=null; v_liquidity text:=null;
  v_pref_fit numeric; v_cap_guard numeric; v_risk_fit numeric; v_goal_fit numeric; v_horizon_fit numeric; v_access_fit numeric; v_div_fit numeric; v_complexity_fit numeric; v_total numeric;
  v_growth numeric; v_income numeric; v_stability numeric; v_liq numeric; v_div numeric; v_complexity numeric; v_min_horizon int;
  v_official text; v_strengths jsonb; v_watchouts jsonb; v_fit text; v_json jsonb:='[]'::jsonb; v_count int:=0;
  v_model text:='investment-dna-match-v4.0';
begin
  ...
  for d in select * from public.v_investment_dna_v1 where data_quality_status='verified' and official_risk_rating is not null order by symbol loop
    ...
    v_growth:=coalesce(d.growth_score,50); v_income:=coalesce(d.income_score,50); v_stability:=coalesce(d.stability_score,50);
    ...
    -- Investor tolerance and issuer risk classification are different scales. Use a categorical compatibility matrix instead of subtracting point scores.
    ...
    -- Capacity is a guardrail, not a target. It mainly prevents financially fragile profiles from ranking higher-risk products too highly.
    ...
    if c.id is null or v_goal is null then v_goal_fit:=70;
      ...
      when v_goal in ('house_purchase','education','major_purchase') then v_stability*.80+coalesce(100-d.equity_pct,50)*.20
      else 70 end;
    end if;
    ...
    elsif v_liquidity in ('very_high','high') then v_access_fit:=v_stability*.80+v_liq*.20;
    ...
    v_total:=v_risk_fit*.35+v_goal_fit*.25+v_horizon_fit*.15+v_access_fit*.10+v_div_fit*.10+v_complexity_fit*.05;
    ...
    if v_cap_guard<70 then v_watchouts:=v_watchouts||jsonb_build_array('This risk category may be more than your current financial capacity can comfortably absorb.'); end if;
    ...
    values(p_assessment_id,d.investment_id,v_model,round(v_total,2),round(v_pref_fit,2),round(v_cap_guard,2),v_official,
      jsonb_build_object('engine',v_model,'signal_type','official-risk-plus-investment-dna-compatibility','not_investment_advice',true,
        'weights',jsonb_build_object('official_risk_compatibility',35,'goal_role',25,'horizon',15,'access_stability',10,'diversification',10,'complexity',5),
        'risk_method','categorical compatibility matrix; Investor DNA and CSA issuer risk rating are not treated as the same numerical scale'),
      jsonb_build_object('fit_label',v_fit,
        ...
        'scores',jsonb_build_object('overall',round(v_total,2),'official_risk_fit',round(v_risk_fit,2),'risk_preference_compatibility',round(v_pref_fit,2),'capacity_guard',round(v_cap_guard,2),'goal_fit',round(v_goal_fit,2),'horizon_fit',round(v_horizon_fit,2),'access_fit',round(v_access_fit,2),'diversification_fit',round(v_div_fit,2),'complexity_fit',round(v_complexity_fit,2)),
        'investor',jsonb_build_object('risk_tolerance_level',v_rt_level,'risk_capacity_level',v_rc_level,'experience',round(v_exp,2),'goal',v_goal,'horizon_months',v_horizon,'liquidity_need',v_liquidity),
        'investment_dna',jsonb_build_object('official_risk_rating',v_official,'official_risk_source',d.official_risk_source_title,'official_risk_source_date',d.official_risk_source_date,'growth',v_growth,'income',v_income,'stability',v_stability,'diversification',v_div,'complexity',v_complexity,'minimum_horizon_months',v_min_horizon,'equity_pct',d.equity_pct),
...
returns jsonb language sql set search_path to 'public','extensions' as $function$
 select public.calculate_investment_match_v4(p_assessment_id);
$function$;
...
declare v_limit int:=greatest(1,least(coalesce(p_limit,10),25)); v_model text:='investment-dna-match-v4.0'; v_count int; v_top jsonb; v_alt jsonb; v_consider jsonb; v_mismatch jsonb;
begin
