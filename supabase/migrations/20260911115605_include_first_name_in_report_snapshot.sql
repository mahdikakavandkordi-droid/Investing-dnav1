...
begin
 ...
 p := jsonb_build_object('assessment_id',p_assessment_id,'report_version','report-v1.2','model_version',r.model_version,'generated_at',now(),'archetype',r.archetype,'risk_tolerance',r.risk_tolerance,'risk_capacity',r.risk_capacity,'behavioral_profile',r.behavioral_profile,'fingerprint_code',f.fingerprint_code,'decision_style',f.decision_style,'pressure_style',f.pressure_style,'strengths',f.strengths,'watchouts',f.watchouts,'narrative',n.narrative,'investment_context',case when c.id is null then null else jsonb_build_object('first_name',c.first_name,'age',c.age,'amount_to_invest',c.amount_to_invest,'amount_currency',c.amount_currency,'goal',c.goal,'time_horizon',c.time_horizon,'liquidity_need',c.liquidity_need,'required_return',c.required_return,'loss_consequence',c.loss_consequence,'experience',c.experience) end);
 insert into public.report_snapshots(assessment_id,report_version,model_version,report) values(p_assessment_id,'report-v1.2',r.model_version,p) on conflict (assessment_id,report_version,model_version) do update set report=excluded.report,generated_at=now() returning id into rid;
