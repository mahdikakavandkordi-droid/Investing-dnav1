delete from public.market_data_source_routing;
insert into public.market_data_source_routing(source_id,asset_type,country_code,exchange,priority_override,is_active)
select id,'ETF','US','NYSEARCA',10,true from public.market_data_sources where source_key='massive';
insert into public.market_data_source_routing(source_id,asset_type,country_code,exchange,priority_override,is_active)
select id,'ETF','CA','TSX',20,true from public.market_data_sources where source_key='internal_verified';
insert into public.market_data_source_routing(source_id,asset_type,country_code,exchange,priority_override,is_active)
select id,'ETF','CA','TSX',30,true from public.market_data_sources where source_key='issuer_primary';
create or replace function public.resolve_market_data_source(p_investment_id uuid,p_data_type text default 'price_history') returns jsonb language plpgsql security definer set search_path=public as $$
...
begin
 ...
 into v_source
 from market_data_sources s join market_data_source_routing r on r.source_id=s.id
 where s.is_active and r.is_active and (r.asset_type is null or r.asset_type=v_asset) and (r.country_code is null or r.country_code=v_country) and (r.exchange is null or r.exchange=v_exchange)
