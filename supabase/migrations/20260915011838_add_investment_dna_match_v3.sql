...
begin
  ...
  for d in select * from public.v_investment_dna_v1 where data_quality_status='verified' order by symbol loop
    ...
    -- Being above willingness/capacity is penalized much more than being below it.
    v_tol_fit:=greatest(0,least(100,100-greatest(0,v_risk-v_rt)*1.70-greatest(0,v_rt-v_risk)*0.35));
    v_cap_fit:=greatest(0,least(100,100-greatest(0,v_risk-v_rc)*2.10-greatest(0,v_rc-v_risk)*0.20));
    v_risk_fit:=v_tol_fit*.55+v_cap_fit*.45;
    ...
    -- Severe horizon mismatch is a compatibility ceiling, not a soft suggestion.
    ...
    if v_risk_fit>=80 then v_strengths:=v_strengths||jsonb_build_array('Its risk level sits close to what your profile can reasonably handle.'); end if;
    ...
    values(p_assessment_id,d.investment_id,v_model,round(v_total,2),round(v_tol_fit,2),round(v_cap_fit,2),d.risk_band,
      ...
      jsonb_build_object('fit_label',v_fit,
        ...
        'scores',jsonb_build_object('overall',round(v_total,2),'risk_fit',round(v_risk_fit,2),'goal_fit',round(v_goal_fit,2),'horizon_fit',round(v_horizon_fit,2),'access_fit',round(v_access_fit,2),'diversification_fit',round(v_div_fit,2),'complexity_fit',round(v_complexity_fit,2)),
