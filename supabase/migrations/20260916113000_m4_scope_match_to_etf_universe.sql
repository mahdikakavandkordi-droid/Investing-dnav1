...
-- boundary must contain ETFs only. Without this filter, GICs, bonds, T-Bills,
-- Commercial Paper and ABCP entered the Match payload as review_required rows
-- and could invalidate the ETF Match cache/data_version despite being outside
-- the current Match product scope.
...
-- Preserve the hardened service-only read boundary.
